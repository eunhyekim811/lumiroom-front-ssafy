...
const createPropertyMarkerImage = () => {
  const svg = `<svg width="38" height="46" viewBox="0 0 38 46" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M19 45C19 45 35 28.5 35 17C35 8.2 27.8 1 19 1C10.2 1 3 8.2 3 17C3 28.5 19 45 19 45Z" fill="#111827" stroke="#FACC15" stroke-width="3"/><path d="M12 17L19 11L26 17V26H12V17Z" fill="#FACC15"/><path d="M17 26V20H21V26" fill="#111827"/></svg>`.trim()
  return new window.kakao.maps.MarkerImage(`data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`, new window.kakao.maps.Size(38, 46), { offset: new window.kakao.maps.Point(19, 46) })
}
...
const renderPropertyMarkers = () => {
  ...
  propertyStore.propertiesList.forEach((property) => {
    ...
    const marker = new window.kakao.maps.Marker({
      map: mapInstance, position: new window.kakao.maps.LatLng(property.lat, property.lng), image: markerImage, title: property.title
    })
    window.kakao.maps.event.addListener(marker, 'click', () => { handlePropertySelect(property) })
